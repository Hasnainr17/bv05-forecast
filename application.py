from flask import Flask, render_template, request, session, send_from_directory, Markup
import uuid
...
def run_validation():
    # Pull the parameters the user just submitted from the HTML form.
    ...
    try:
        # Check Ensure the user didn't use a city not in the known list
        if city not in LOCATIONS:
            raise ValueError(f"City '{city}' not supported.")

        # Locate the historical data for the chosen city and train the OLS models
        hist_path = DATA_DIR / LOCATIONS[city]
        res_model, ci_model = train_models_from_historical_csv(hist_path)

        # Run the math comparing actual vs predicted load, and save it to Excel
        perform_validation(
            res_model=res_model,
            ci_model=ci_model,
        ...
        # Call the Plotly module to generate the interactive HTML graphs
        validation_html = get_validation_section(city, start_date, end_date)

    except Exception as e:
        # Generate a safe HTML warning banner to display to the user.
    ...
    # Because we are returning the user to index.html, we must regenerate 
    # All the data required for the other tabs, otherwise they will load blank
    load_data = build_load_forecast(city)
    weather_data = build_weather(city, load_data)

    # Extract the correct latest_load and next_day for the UI cards
    if load_data and len(load_data) >= 2:
        latest_load = load_data[0]
        next_day = load_data[1]
    else:
        # Fallback if the forecast engine fails
        latest_load = {"temperature": "N/A", "forecast_residential_load": 0, "forecast_ci_load": 0}
        next_day = {"temperature": "N/A", "forecast_residential_load": 0, "forecast_ci_load": 0}

    # Pull from the session cookie to keep the 5 day manual inputs intact
    default_dates = [(datetime.today() + timedelta(days=i)).strftime("%Y-%m-%d") for i in range(5)]
    user_submitted_data = session.get('user_data', [{"date": d, "temp": "", "wind": ""} for d in default_dates])

    # Passes the generated Plotly charts to the frontend website
    return render_template(
        "index.html",
        selected_city=city,
        cities=CITIES,
